...
module LogStash module Filters module Geoip class DatabaseMetric
  ...
  def initialize_metrics(metadatas, states)
    metadatas.each do |row|
      ...
      @metric.namespace([:database, type.to_sym]).tap do |n|
        n.gauge(:status, states[type].is_eula ? DATABASE_UP_TO_DATE : DATABASE_CC)
  ...
  def set_download_error(error_msg)
    @metric.namespace([:download_stats]).gauge(:last_failed_at, Time.now.iso8601)
    @metric.namespace([:download_stats]).gauge(:last_error_msg, error_msg)
  end
