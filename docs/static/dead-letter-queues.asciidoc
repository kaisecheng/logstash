...
change this setting, use the `dead_letter_queue.max_bytes` option.
Dead letter queues do not have file rotation in runtime. DLQs accumulate entries until reaching the maximum size.
Entries will be dropped if they would exceed the size.

Old files in the dead letter queues do not get deleted automatically.
